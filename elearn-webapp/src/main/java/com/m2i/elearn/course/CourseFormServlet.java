...
public class CourseFormServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(titre.length()<=10 && titre!=null){
			throw new RuntimeException("Titre invalide 10 caractère minimum");
		}
		if(description.length()<=140 && description!=null){//THIB remplacer 40 caractère par 20 mots
			throw new RuntimeException("Description invalide 20 mots minimum");
		}
		
		
		
		if(tag.length()<=3 ){
			throw new RuntimeException("Tag invalide 3 caractère minimum");
		}
